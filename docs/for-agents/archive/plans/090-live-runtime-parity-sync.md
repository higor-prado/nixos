# Live Runtime Parity Sync for Waybar, Notifications, and Clipboard

## Goal

Make the tracked repo reflect the current working live desktop state on `predator` for the mutable desktop surfaces we have actively tuned in this conversation: Waybar, the clipboard picker, and notification controls/Mako behavior. The execution model for this work is live-first: inspect and, when needed, edit the live runtime files first, then sync those results back into repo-owned sources.
...
Out of scope:
- Hyprland config migration and parity work
...
- Hyprland parity/migration work has been split into a separate planning track because the Lua-config transition in upstream Hyprland changes the right target shape for repo ownership.
...
- Therefore repo-vs-live parity is currently incomplete for Waybar/Mako/clipboard even after the recent fixes.

## Desired End State

- Repo sources match the current working live desktop state for Waybar, clipboard picker, and notification controls.
...
- repo owners under `config/apps/waybar/**`, `modules/features/desktop/mako.nix`
...
### Phase 3: Parity verification and drift guard
...
- Repo reflects the current live state for the requested Waybar, clipboard, and notification surfaces.
