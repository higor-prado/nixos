...
expect_equal "predator gdm feature" "$(bool_eval "path:$PWD#nixosConfigurations.predator.config.services.displayManager.gdm.enable")" "true"
expect_equal "predator gdm default session" "$(nix eval --raw "path:$PWD#nixosConfigurations.predator.config.services.displayManager.defaultSession")" "hyprland-uwsm"
expect_equal "predator gdm single session package" "$(host_cfg_expr "predator" 'builtins.length cfg.services.displayManager.sessionPackages == 1')" "true"
expect_equal "predator gdm monitor layout" "$(host_cfg_expr "predator" 'builtins.hasAttr "xdg/monitors.xml" cfg.environment.etc')" "true"
expect_equal "predator gdm monitor layout source exists" "$(host_cfg_expr "predator" 'builtins.pathExists cfg.environment.etc."xdg/monitors.xml".source')" "true"
expect_equal "predator gdm shell greeter monitor patch" "$(nix_eval_json_expr 'let pkgs = (builtins.getFlake "path:'"${PWD}"'").nixosConfigurations.predator.pkgs; in builtins.any (patch: baseNameOf (toString patch) == "gnome-shell-gdm-login-dialog-largest-monitor.patch") (pkgs.gnome-shell.patches or [ ])' | jq -r '.')" "true"

predator_gdm_monitors_xml="config/desktops/gdm/predator-monitors.xml"
if [ ! -f "$predator_gdm_monitors_xml" ]; then
	report_fail "predator gdm monitor XML missing at ${predator_gdm_monitors_xml}"
else
	for expected_fragment in \
		...
		"<vendor>CMN</vendor>" \
		"<serial>0x00000000</serial>"; do
		if ! grep -Fq "$expected_fragment" "$predator_gdm_monitors_xml"; then
			report_fail "predator gdm monitor XML missing '${expected_fragment}'"
		fi
	done
fi
...
expect_equal "predator keyrs restricted to selected user" "$(host_cfg_expr "predator" "cfg.systemd.user.services.keyrs.unitConfig.ConditionUser == \"${predator_hm_user}\"")" "true"
...
expect_equal "aurelius gdm feature" "$(host_cfg_expr "aurelius" 'if builtins.hasAttr "displayManager" cfg.services then cfg.services.displayManager.gdm.enable else false')" "false"
expect_equal "aurelius gdm monitor layout absent" "$(host_cfg_expr "aurelius" '!(builtins.hasAttr "xdg/monitors.xml" cfg.environment.etc)')" "true"
...
expect_equal "cerebelo gdm monitor layout absent" "$(host_cfg_expr "cerebelo" '!(builtins.hasAttr "xdg/monitors.xml" cfg.environment.etc)')" "true"
