...
printf -- '- Branch: `%s`\n' "$(git rev-parse --abbrev-ref HEAD)" >>"$results_dir/summary.md"
printf -- '- Commit: `%s`\n' "$(git rev-parse --short HEAD)" >>"$results_dir/summary.md"
printf -- '- Host: `%s`\n' "$host" >>"$results_dir/summary.md"
printf -- '- Timestamp: `%s`\n\n' "$timestamp" >>"$results_dir/summary.md"
...
nix shell nixpkgs#hyperfine nixpkgs#jq -c bash -lc "
  set -euo pipefail
  hyperfine --warmup 1 --runs 5 --export-json '$results_dir/build-eval.json' \
...
if scripts/check-runtime-smoke.sh --strict-backends >"$results_dir/runtime-health/smoke.txt" 2>&1; then
